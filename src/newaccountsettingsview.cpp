...
struct _NewAccountSettingsViewPrivate
{
    ...
    GtkWidget* sip_allow_contact_rewrite_row;
        GtkWidget* combobox_sip_allow_contact_rewrite;
    GtkWidget* sip_contact_rewrite_method_row;
        GtkWidget* combobox_sip_contact_rewrite_method;
...
new_account_settings_view_class_init(NewAccountSettingsViewClass *klass)
{
    ...
    gtk_widget_class_bind_template_child_private(GTK_WIDGET_CLASS(klass), NewAccountSettingsView, sip_allow_contact_rewrite_row);
    gtk_widget_class_bind_template_child_private(GTK_WIDGET_CLASS(klass), NewAccountSettingsView, combobox_sip_allow_contact_rewrite);
    gtk_widget_class_bind_template_child_private(GTK_WIDGET_CLASS(klass), NewAccountSettingsView, sip_contact_rewrite_method_row);
    gtk_widget_class_bind_template_child_private(GTK_WIDGET_CLASS(klass), NewAccountSettingsView, combobox_sip_contact_rewrite_method);
...
static void
allow_contact_rewrite_changed(NewAccountSettingsView *view)
{
    if (!is_config_ok(view))
        return;

    auto* priv = NEW_ACCOUNT_SETTINGS_VIEW_GET_PRIVATE(view);
    GtkComboBoxText* level = GTK_COMBO_BOX_TEXT(priv->combobox_sip_allow_contact_rewrite);
    auto* text = gtk_combo_box_text_get_active_text(level);
    if (!text)
        return;

    std::string activeText = text;
    auto newState = lrc::api::account::AllowContactRewrite::ENABLE;
    if (activeText == "DISABLE") {
        newState = lrc::api::account::AllowContactRewrite::DISABLE;
    } else if (activeText == "ALWAYS") {
        newState = lrc::api::account::AllowContactRewrite::ALWAYS;
    }
    g_free(text);

    if (newState != priv->currentProp_->allowContactRewrite) {
        priv->currentProp_->allowContactRewrite = newState;
        new_account_settings_view_save_account(view);
    }
}

static void
contact_rewrite_method_changed(NewAccountSettingsView *view)
{
    if (!is_config_ok(view))
        return;

    auto* priv = NEW_ACCOUNT_SETTINGS_VIEW_GET_PRIVATE(view);
    GtkComboBoxText* method = GTK_COMBO_BOX_TEXT(priv->combobox_sip_contact_rewrite_method);
    auto* text = gtk_combo_box_text_get_active_text(method);
    if (!text)
        return;

    std::string activeText = text;
    auto newState = lrc::api::account::ContactRewriteMethod::NO_UNREG;
    if (activeText == "UNREGISTER") {
        newState = lrc::api::account::ContactRewriteMethod::UNREGISTER;
    }
    g_free(text);

    if (newState != priv->currentProp_->contactRewriteMethod) {
        priv->currentProp_->contactRewriteMethod = newState;
        new_account_settings_view_save_account(view);
    }
}
...
new_account_settings_view_update(NewAccountSettingsView *view, gboolean reset_view)
{
    ...
    if ((*priv->accountInfo_)->profileInfo.type == lrc::api::profile::Type::RING) {
        ...
        gtk_widget_hide(priv->sip_allow_contact_rewrite_row);
        gtk_widget_hide(priv->sip_contact_rewrite_method_row);
        ...
        gtk_widget_show_all(priv->sip_allow_contact_rewrite_row);
        gtk_widget_show_all(priv->sip_contact_rewrite_method_row);
        ...
        gtk_combo_box_text_remove_all(GTK_COMBO_BOX_TEXT(priv->combobox_sip_allow_contact_rewrite));
        gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(priv->combobox_sip_allow_contact_rewrite), nullptr, "DISABLE");
        gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(priv->combobox_sip_allow_contact_rewrite), nullptr, "ENABLE");
        gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(priv->combobox_sip_allow_contact_rewrite), nullptr, "ALWAYS");
        switch (priv->currentProp_->allowContactRewrite) {
        case lrc::api::account::AllowContactRewrite::DISABLE:
            gtk_combo_box_set_active(GTK_COMBO_BOX(priv->combobox_sip_allow_contact_rewrite), 0);
            break;
        case lrc::api::account::AllowContactRewrite::ENABLE:
            gtk_combo_box_set_active(GTK_COMBO_BOX(priv->combobox_sip_allow_contact_rewrite), 1);
            break;
        case lrc::api::account::AllowContactRewrite::ALWAYS:
        default:
            gtk_combo_box_set_active(GTK_COMBO_BOX(priv->combobox_sip_allow_contact_rewrite), 2);
            break;
        }
        g_signal_connect_swapped(priv->combobox_sip_allow_contact_rewrite, "changed", G_CALLBACK(allow_contact_rewrite_changed), view);

        gtk_combo_box_text_remove_all(GTK_COMBO_BOX_TEXT(priv->combobox_sip_contact_rewrite_method));
        gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(priv->combobox_sip_contact_rewrite_method), nullptr, "UNREGISTER");
        gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(priv->combobox_sip_contact_rewrite_method), nullptr, "UPDATE");
        switch (priv->currentProp_->contactRewriteMethod) {
        case lrc::api::account::ContactRewriteMethod::UNREGISTER:
            gtk_combo_box_set_active(GTK_COMBO_BOX(priv->combobox_sip_contact_rewrite_method), 0);
            break;
        case lrc::api::account::ContactRewriteMethod::NO_UNREG:
            gtk_combo_box_set_active(GTK_COMBO_BOX(priv->combobox_sip_contact_rewrite_method), 1);
        default:
            gtk_combo_box_set_active(GTK_COMBO_BOX(priv->combobox_sip_contact_rewrite_method), 1);
            break;
        }
        g_signal_connect_swapped(priv->combobox_sip_contact_rewrite_method, "changed", G_CALLBACK(contact_rewrite_method_changed), view);
