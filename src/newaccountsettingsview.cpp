...
static void
enable_custom_ringtone(GtkWidget*, NewAccountSettingsView* view)
{
    g_return_if_fail(IS_NEW_ACCOUNT_SETTINGS_VIEW(view));
    auto* priv = NEW_ACCOUNT_SETTINGS_VIEW_GET_PRIVATE(view);
    ...
    auto newState = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(priv->custom_ringtone_button));
...
static void
enable_custom_address(GtkWidget*, NewAccountSettingsView *view)
{
    g_return_if_fail(IS_NEW_ACCOUNT_SETTINGS_VIEW(view));
    auto* priv = NEW_ACCOUNT_SETTINGS_VIEW_GET_PRIVATE(view);
    ...
    auto newState = !gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(priv->button_custom_published));
...
build_settings_view(NewAccountSettingsView* view)
{
    ...
    css += ".settings-frame { background: white; padding: 20px; border-radius: 10px; }";
    ...
    g_signal_connect(priv->button_custom_published, "toggled", G_CALLBACK(enable_custom_address), view);
...
new_account_settings_view_update(NewAccountSettingsView *view)
{
    ...
    if ((*priv->accountInfo_)->profileInfo.type == lrc::api::profile::Type::RING) {
        ...
        // Clear username box
        auto* children = gtk_container_get_children(GTK_CONTAINER(priv->box_bootstrap_servers));
        if (children) {
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(priv->button_custom_published), !priv->currentProp_->publishedSameAsLocal);
    ...
    if (!(*priv->accountInfo_)->registeredName.empty()) {
        ...
        auto* register_button = gtk_button_new();
