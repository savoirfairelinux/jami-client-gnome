...
struct _NewAccountSettingsViewPrivate
{
    ...
    GtkWidget* sip_encrypt_negociation;
...
new_account_settings_view_class_init(NewAccountSettingsViewClass *klass)
{
    ...
    gtk_widget_class_bind_template_child_private(GTK_WIDGET_CLASS(klass), NewAccountSettingsView, sip_encrypt_negociation);
...
enable_codec(GObject* switch_btn, GParamSpec*, NewAccountSettingsView *view)
{
    ...
    unsigned int* id = (unsigned int*)g_object_get_data(switch_btn, "id");
...
draw_codecs(NewAccountSettingsView* view)
{
    ...
    // Clear video codecs
    auto* children = gtk_container_get_children(GTK_CONTAINER(priv->list_video_codecs));
    if (children) {
        auto* current_child = g_list_first(children);
        while (current_child) {
            auto* future_child = g_list_next(current_child);
            gtk_container_remove(GTK_CONTAINER(priv->list_video_codecs), GTK_WIDGET(current_child->data));
    ...
    // Clear audio codecs
    children = gtk_container_get_children(GTK_CONTAINER(priv->list_audio_codecs));
    if (children) {
        auto* current_child = g_list_first(children);
        while (current_child) {
            auto* future_child = g_list_next(current_child);
            gtk_container_remove(GTK_CONTAINER(priv->list_audio_codecs), GTK_WIDGET(current_child->data));
            current_child = future_child;
        }
    }
    // Add audio codecs
...
up_video_priority_clicked(NewAccountSettingsView* view)
{
    ...
    if (children) {
        ...
        unsigned int* id = (unsigned int*)g_object_get_data(G_OBJECT(switch_btn), "id");
...
new_account_settings_view_update(NewAccountSettingsView *view)
{
    ...
    if ((*priv->accountInfo_)->profileInfo.type == lrc::api::profile::Type::RING) {
        ...
        gtk_container_add(GTK_CONTAINER(priv->username_box), GTK_WIDGET(username_registration_widget));
        ...
        gtk_widget_hide(priv->sip_encrypt_negociation);
        ...
        gtk_widget_show_all(priv->sip_encrypt_negociation);
