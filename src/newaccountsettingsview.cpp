...
on_show_banned(GtkToggleButton*, NewAccountSettingsView* view)
{
    ...
    if (active) {
        ...
        auto image = gtk_image_new_from_resource("/cx/ring/RingGnome/up_arrow");
        ...
        auto image = gtk_image_new_from_resource("/cx/ring/RingGnome/bottom_arrow");
        gtk_button_set_image(GTK_BUTTON(priv->button_show_banned), image);
...
add_banned(NewAccountSettingsView *view, const std::string& contactUri)
{
    ...
    // Add action button
    auto image = gtk_image_new_from_resource("/cx/ring/RingGnome/invite");
