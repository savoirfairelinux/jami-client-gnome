...
up_video_priority_clicked(NewAccountSettingsView* view)
{
    ...
    if (children) {
        ...
        if (id) (*priv->accountInfo_)->codecModel->increasePriority(GPOINTER_TO_UINT(id));
    }
}
...
down_video_priority_clicked(NewAccountSettingsView* view)
{
    ...
    if (children) {
        ...
        auto* switch_btn = g_list_last(box)->data;
        unsigned int* id = (unsigned int*)g_object_get_data(G_OBJECT(switch_btn), "id");
        if (id) (*priv->accountInfo_)->codecModel->decreasePriority(GPOINTER_TO_UINT(id));
    }
}
...
new_account_settings_view_update(NewAccountSettingsView *view)
{
    ...
    if ((*priv->accountInfo_)->profileInfo.type == lrc::api::profile::Type::RING) {
        ...
        // Clear video codecs
        children = gtk_container_get_children(GTK_CONTAINER(priv->list_video_codecs));
        if (children) {
            auto* current_child = g_list_first(children);
            while (current_child) {
                auto* future_child = g_list_next(current_child);
