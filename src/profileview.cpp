...
build_view(ProfileView* view)
{
    ...
    try
    {
        ...
        if (!conversationOpt || conversationOpt->participants.empty()) return false;
        const auto& contact = (*priv->accountInfo_)->contactModel->getContact(conversationOpt->participants.front());
