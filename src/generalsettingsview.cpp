...
clear_history(G_GNUC_UNUSED GtkWidget *button, GeneralSettingsView *self)
{
    ...
    if (clear_history_dialog(self) ) {
        ...
        //~ if (priv->accountContainer_->info)
            qDebug() << "toto : " << priv->accountContainer_->info.id.c_str();
            //~ priv->accountContainer_->info.conversationModel->clearAllHistory();
        //~ else
            //~ g_warning("clear_history, priv->accountContainer_ invalid");
    }
}
