...
update(GtkTreeSelection *selection, ContactPopupMenu *self)
{
    ...
    /* build a new menu, unless there is no item selected */
    GtkTreeIter iter;
    GtkTreeModel *model;
    ...
    if (type.isValid() && object.isValid()) {
        switch (type.value<Ring::ObjectType>()) {
            case Ring::ObjectType::Person:
            {
                ...
                if (cms.size() == 1) {
                    ...
                    gtk_menu_shell_append(GTK_MENU_SHELL(self), copy_item);
     ...
     if (type.isValid() && object.isValid()) {
         if (type.value<Ring::ObjectType>() == Ring::ObjectType::ContactMethod) {
             ...
             auto add_to = menu_item_add_to_contact(object.value<ContactMethod *>(), GTK_WIDGET(priv->treeview), &rect);
             gtk_menu_shell_append(GTK_MENU_SHELL(self), add_to);
         }
     }
...
static void
contact_popup_menu_init(ContactPopupMenu *self)
{
    // nothing to do
}
