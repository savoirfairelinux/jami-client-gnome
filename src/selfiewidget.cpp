...
static void
on_view_signal(SelfieWidget *self) {
...
selfie_widget_init(SelfieWidget *self)
{
    ...
    g_signal_connect_swapped(priv->selfie_view, "quit_action", G_CALLBACK(on_view_signal), self);
    g_signal_connect_swapped(priv->selfie_view, "send_action", G_CALLBACK(on_view_signal), self);
