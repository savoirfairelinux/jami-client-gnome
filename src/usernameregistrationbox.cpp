        ...
        [=] (const Account*, NameDirectory::LookupStatus status, const QString&, const QString& name) {
            ...
            switch(status)
            {
                case NameDirectory::LookupStatus::SUCCESS:
                {
                    ...
                    gtk_label_set_text(GTK_LABEL(priv->label_status), g_strdup_printf(_("Username is not available")));
                ...
                case NameDirectory::LookupStatus::INVALID_NAME:
                {
                    ...
                    gtk_label_set_text(GTK_LABEL(priv->label_status), g_strdup_printf(_("Username is not valid")));
                    gtk_widget_show(priv->icon_username_availability);
                    g_signal_emit(G_OBJECT(view), username_registration_box_signals[USERNAME_AVAILABILITY_CHANGED], 0, FALSE);
                    break;
                }
                case NameDirectory::LookupStatus::NOT_FOUND:
                {
                    ...
                    gtk_label_set_text(GTK_LABEL(priv->label_status),g_strdup_printf(_("Username is available")));
                    gtk_widget_show(priv->icon_username_availability);
                ...
                case NameDirectory::LookupStatus::ERROR:
                {
                    ...
                    gtk_label_set_text(GTK_LABEL(priv->label_status), g_strdup_printf(_("Could not lookup username")));
                    gtk_widget_show(priv->icon_username_availability);
                    g_signal_emit(G_OBJECT(view), username_registration_box_signals[USERNAME_AVAILABILITY_CHANGED], 0, FALSE);
                    break;
                }
