...
{
    /* This check is needed as the function might be triggered after the view
     * is destroyed.
     */
    ...
    priv->registered_name_found = QObject::connect(
        ...
        [=] (G_GNUC_UNUSED const Account* account, NameDirectory::LookupStatus status, G_GNUC_UNUSED const QString& address, const QString& name) {
            ...
            switch(status)
            {
                ...
                case NameDirectory::LookupStatus::INVALID_NAME:
                {
                    gtk_widget_set_sensitive(priv->button_register_username, FALSE);
                    gtk_image_set_from_icon_name(GTK_IMAGE(priv->icon_username_availability), "error", GTK_ICON_SIZE_SMALL_TOOLBAR);
                    gtk_widget_set_tooltip_text(priv->icon_username_availability, _("The entered username is not nalid"));
                    gtk_label_set_text(GTK_LABEL(priv->label_status), g_strdup_printf(_("Username \"%s\" is not valid"), name.toStdString().c_str()));
                    gtk_widget_show(priv->icon_username_availability);
                    g_signal_emit(G_OBJECT(view), username_registration_box_signals[USERNAME_AVAILABILITY_CHANGED], 0, FALSE);
                    break;
                }
    ...
    NameDirectory::instance().lookupName(nullptr, QString(), *priv->username_waiting_for_lookup_result);
...
entry_username_changed(G_GNUC_UNUSED GtkEntry* entry, UsernameRegistrationBox *view)
{
    ...
    g_timeout_add_full(G_PRIORITY_DEFAULT, 3000, (GSourceFunc)lookup_username, view, NULL);
}
...
static void
build_view(UsernameRegistrationBox *view, bool register_button, bool multi_line)
{
    UsernameRegistrationBoxPrivate *priv = USERNAME_REGISTRATION_BOX_GET_PRIVATE(view);
...
GtkWidget *
username_registration_box_new(Account* account, gboolean register_button, gboolean multi_line)
{
    ...
    build_view(USERNAME_REGISTRATION_BOX(view), register_button, multi_line);
