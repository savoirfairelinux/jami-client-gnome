...
create_new_ring_account(AccountCreationWizard *win)
{
    ...
    //TODO: Free those strings
    const gchar *alias = g_strdup(gtk_entry_get_text(GTK_ENTRY(priv->entry_alias)));

    const gchar *password = g_strdup(gtk_entry_get_text(GTK_ENTRY(priv->entry_password)));
