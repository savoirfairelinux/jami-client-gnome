...
display_technical_information()
{
    ...
    /*QObject::connect(&CallModel::instance(), &CallModel::updateInfo,
        [lblLocalFps, lblRemoteFps] (MapStringString info) {
            ...
            gtk_label_set_text(GTK_LABEL(lblLocalFps),g_strdup_printf("%s %s","Local frame rate: ", info["local FPS"].toStdString().c_str()));
            qDebug()<<QString::number(SmartInfoHub::instance().getLocalFps());
        ...
        QObject::connect(&SmartInfoHub::instance(), &SmartInfoHub::changed,
            [] () {
                qDebug() << SmartInfoHub::instance().localFps();
        });
