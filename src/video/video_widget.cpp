...
// check if the space above is required
...
clutter_render_image(VideoWidgetRenderer* wg_renderer)
{
...

    {
        ...
        if (wg_renderer->take_a_photo) {
            ...
            gdk_pixbuf_save_to_buffer(pixbuf_frame_resized, &png_buffer, &png_buffer_size, "png", NULL, NULL);

            /* save on disk for test only */
            gdk_pixbuf_save (pixbuf_frame_resized, "test.png", "png", NULL, NULL); // rm me

            if(ProfileModel::instance().selectedProfile())
                ProfileModel::instance().selectedProfile()->person()->setPhoto(png_buffer);
