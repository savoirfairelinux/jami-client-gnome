...
typedef struct _VideoInfo {
    ...
    ClutterRenderer *clutter_renderer;
    gboolean        is_mixer;
} VideoInfo;
...
static gboolean   is_video_in_screen                    (GtkWidget *, const char *);
static VideoInfo*     video_widget_retrieve_camera          (GtkWidget *, VIDEO_AREA_ID);
static void       video_widget_add_camera_in_screen     (GtkWidget *, VIDEO_AREA_ID, VideoInfo *);
...
video_widget_init(VideoWidget *self)
{
    ...
    for (int i = 0; i < VIDEO_AREA_LAST; i++) {
        video_area = video_widget_video_area_get(GTK_WIDGET(self), (VIDEO_AREA_ID)i);
...
video_widget_redraw_screen(GtkWidget *self)
{
    ...
    VideoInfo *camera_remote = video_widget_retrieve_camera(self, VIDEO_AREA_REMOTE);
    VideoInfo *camera_local  = video_widget_retrieve_camera(self, VIDEO_AREA_LOCAL);
    ...
    if (video_area_local && video_area_local->show && camera_local) {
        ...
        /* else the local camera must be resize keeping the aspect ratio and placed */
            aspect_ratio = (gdouble) camera_local->renderer->size().width()
                           / camera_local->renderer->size().height();
                             ...
                             VIDEO_AREA_ID video_area_id)
{
    ...
    VideoInfo *video = (VideoInfo *)g_hash_table_lookup(priv->video_handles, video_area->video_id);
    if (!video)
                                  ...
                                  VideoInfo *video)
{
    ...
    VideoArea *video_area = video_widget_video_area_get(self, video_area_id);
    if (video_area) {
        video_area->video_id = g_strdup(video->renderer->id().toLocal8Bit().constData());
                                     ...
                                     VIDEO_AREA_ID video_area_id)
{
    ...
    VideoInfo *video = video_widget_retrieve_camera(self, video_area_id);
    if (video) {
        ...
        if (clutter_actor_contains(priv->video_screen.container, video->texture)) {
             ...
             * the actor */
            // video_renderer_stop(video->clutter_renderer);
            g_object_unref(G_OBJECT(video->clutter_renderer));
        ...
        G_GNUC_UNUSED gpointer data)
{
    ...
    if (uris && *uris) {
        /* TODO: change input to video */
       // sflphone_switch_video_input(*uris);
        g_debug("dragged video source into video widget");
    }
    g_strfreev(uris);
}
                                   ...
                                   gpointer data)
{
    ...
    if (event->type == GDK_2BUTTON_PRESS) {
        ...
        g_debug("double click in video widget");
                          ...
                          gboolean is_mixer)
{
    ...
    VideoInfo *video = g_new0(VideoInfo, 1);
    ...
    video->clutter_renderer = clutter_renderer_new(video->texture, video->renderer);

    video->is_mixer = is_mixer;

    /* add the video to the video list */
    g_hash_table_insert(priv->video_handles, g_strdup(renderer->id().toLocal8Bit().constData()), video);
                         ...
                         Video::Renderer *renderer)
{
    ...
    const char* video_id = renderer->id().toLocal8Bit().constData();
    if (!(video = (VideoInfo *)g_hash_table_lookup(priv->video_handles, video_id))) {
