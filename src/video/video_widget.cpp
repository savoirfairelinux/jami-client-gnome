...
video_widget_dispose(GObject *object)
{
    ...
    if (priv->timer_fade) g_source_remove(priv->timer_fade);
...
on_show_actions_popover(GtkButton *button, VideoWidget *self)
{
    ...
    if (!isLocal) {
        ...
        gtk_box_pack_start(GTK_BOX(box), hangupBtn, FALSE, TRUE, 0);
...
static gboolean
on_timer_fade_timeout(VideoWidget* view)
{
    g_warning("@@@");
    VideoWidgetPrivate *priv = VIDEO_WIDGET_GET_PRIVATE(view);
    g_return_val_if_fail(priv, G_SOURCE_REMOVE);

    for (const auto& [uri, actor]: priv->cpp->hovers_) {
        if (gtk_widget_is_visible(priv->actions_popover)) {
            QString uri = QString::fromStdString((gchar*)g_object_get_data(G_OBJECT(priv->actions_popover), "uri"));
            QString uriActor = QString::fromStdString((gchar*)g_object_get_data(G_OBJECT(actor), "uri"));
            if (uri == uriActor) continue;
        }
...
video_widget_on_event(VideoWidget *self, GdkEvent* event)
{
    ...
    auto posX = event->button.x, posY = event->button.y;
    g_warning("@@@ %f %f", posX, posY);
    ...
    if (priv->timer_fade) g_source_remove(priv->timer_fade);
    priv->timer_fade = g_timeout_add(1000, (GSourceFunc)on_timer_fade_timeout, self);
}
...
video_widget_on_button_press_in_screen_event(VideoWidget *self,  GdkEventButton *event, G_GNUC_UNUSED gpointer)
{
    ...
    return FALSE; // event has been fully handled
}
