...
clutter_render_image(VideoWidgetRenderer* wg_renderer)
{
...

    {
        ...
        if (wg_renderer->net) {
            ...
            wg_renderer->net->addImage(frame_data, res.width(), res.height());
            if (auto result = wg_renderer->net->getResult()) {
                ...
                for(int i = 0, j = 0 ; i < res.width() * res.height() * 4 ; i += 4, j += 3 ) {
                    ...
                    {
                    frame_data[i + 2] = yolobox[j + 0]; //return to 0-255 range
                    frame_data[i + 1] = yolobox[j + 1];
                    frame_data[i + 0] = yolobox[j + 2];
                    }
