...
display_technical_information()
{
    ...
    QObject::connect(&CallModel::instance(), &CallModel::updateInfo,
        [lbl] (MapStringString info) {
            //const gchar* affich1 = info["counter"].toStdString().c_str();
            const gchar* fps = "Frame rate: ";
            const gchar* affichFps = info["fps"].toStdString().c_str();
            const gchar* affich = g_strdup_printf("%s %s","Frame rate: ", affichFps);
            //const gchar* affich=g_strdup_printf("%d", val);
            gtk_label_set_text(GTK_LABEL(lbl),affich);
        });
