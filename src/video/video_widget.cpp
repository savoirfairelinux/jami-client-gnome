                      ...
                      Call *call)
{
    ...
    if (uris && *uris){
        call->getOutgoingVideoMedia()->sourceModel()->setFile(QUrl(*uris));
    }
...
switch_video_input_screen(G_GNUC_UNUSED GtkWidget *item, Call* call)
{
    ...
    call->getOutgoingVideoMedia()->sourceModel()->setDisplay(display, QRect(x,y,width,height));
}
...
switch_video_input_file(GtkWidget *item, GtkWidget *parent)
{
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
        ...
        call->getOutgoingVideoMedia()->sourceModel()->setFile(QUrl(uri));
    }
 ...
 */
gboolean
video_widget_on_button_press_in_screen_event(GtkWidget *parent,
                                ...
                                Call* call)
{
    ...
    Video::SourceModel *sourcemodel = call->getOutgoingVideoMedia()->sourceModel();
    if(!sourcemodel){
        return FALSE;
    }
