...
on_set_moderator(GtkButton *button, VideoWidget *self)
{
    ...
    if (conv_opt.has_value()) {
        ...
        call_set_moderator(conv, (*priv->cpp->accountInfo), uri, set_moderator);
    }
}
...
on_show_actions_popover(GtkButton *button, VideoWidget *self)
{
    ...
    if (conv_opt.has_value()) {
        auto conv = conv_opt.value();
        if (call_is_current_host(conv, (*priv->cpp->accountInfo)) &&
            !call_is_host(conv, (*priv->cpp->accountInfo), (gchar*) uri)) {
            bool moderator = call_is_moderator(conv, (*priv->cpp->accountInfo), (gchar*) uri);
            ...
            gtk_style_context_add_class(context, "options-btn");
            auto image = gtk_image_new_from_icon_name("moderator", GTK_ICON_SIZE_BUTTON);
...
video_widget_add_participant_hover(VideoWidget *self, const QJsonObject& participant)
{
    ...
    if ((*priv->cpp->accountInfo)->callModel->isModerator(priv->cpp->callId)) {
