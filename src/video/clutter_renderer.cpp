...
static void clutter_renderer_finalize(GObject *gobject);
...
clutter_renderer_render_to_texture(ClutterRenderer *self)
{
    ...
    const gint BPP = 4;
    const gint ROW_STRIDE = BPP * priv->renderer->size().width();

    /* update the clutter texture */
    GError *error = NULL;
    clutter_texture_set_from_rgb_data(CLUTTER_TEXTURE(priv->texture),
            (guchar*) priv->renderer->currentFrame().constData(),
            TRUE,
            priv->renderer->size().width(),
            priv->renderer->size().height(),
            ROW_STRIDE,
            BPP,
