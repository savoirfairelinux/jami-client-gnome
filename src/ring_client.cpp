...
init_systray(RingClient *client)
{
    ...
    // the menu is managed by the AppIndicator object, so we want our pointer to be set to null when
    // it is destoryed
    g_object_add_weak_pointer(G_OBJECT(priv->icon_menu), (gpointer *)&priv->icon_menu);
