...
#define ACCOUNT_SECURITY_TAB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE ((obj), ACCOUNT_SECURITY_TAB_TYPE, AccountSecurityTabPrivate))
...
static void
render_check_state(GtkCellLayout *cell_layout,
...
build_tab_view(AccountSecurityTab *self)
{
    ...
    /* cannot disable it for RING accounts */
    if (priv->account->protocol() == Account::Protocol::RING)
        gtk_widget_set_sensitive(priv->checkbutton_use_tls, FALSE);
