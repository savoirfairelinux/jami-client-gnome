...
autostart_symlink(gboolean autostart)
{
    ...
    if (autostart) {
        ...
        desktop_paths[0] = g_strconcat(RING_CLIENT_INSTALL, RING_DATA_DIR, "/gnome-ring.desktop", NULL);
        desktop_paths[1] = g_strconcat("/usr", RING_DATA_DIR, "/gnome-ring.desktop", NULL);
        desktop_paths[2] = g_strconcat("/usr/local", RING_DATA_DIR, "/gnome-ring.desktop", NULL);
