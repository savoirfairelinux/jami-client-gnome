    ...
    priv->new_message_connection = QObject::connect(
        ...
        [priv, model] (const QModelIndex &parent, int first, int last) {
            for (int row = first; row <= last; ++row) {
                ...
                g_signal_emit(G_OBJECT(view), chat_view_signals[NEW_MESSAGES_DISPLAYED], 0);
            }
        }
    );
}
...
void
chat_view_set_call_info(ChatView *view, const QModelIndex& idx) {
    ChatViewPrivate *priv = CHAT_VIEW_GET_PRIVATE(view);
...
chat_view_new(void)
{
    return (GtkWidget *)g_object_new(CHAT_VIEW_TYPE, NULL);
}
