...
webkit_chat_container_script_dialog(G_GNUC_UNUSED GtkWidget* webview, gchar *interaction, ChatView* self)
{
    ...
    if (order == "ACCEPT") {
        ...
        try {
            ...
            if (!priv->conversation_) return;
            priv->accountContainer_->info.conversationModel->clearInteractionFromConversation(priv->conversation_->uid, interactionId);
        } catch (...) {
        ...
        try {
            ...
            if (!priv->conversation_) return;
            priv->accountContainer_->info.conversationModel->retryInteraction(priv->conversation_->uid, interactionId);
        } catch (...) {
...
webkit_chat_container_ready(ChatView* self)
{
    ...
    &*priv->accountContainer_->info.conversationModel, &lrc::api::ConversationModel::interactionRemoved,
