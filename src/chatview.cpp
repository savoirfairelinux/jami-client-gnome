...
message_index_to_json_message_object(const QModelIndex &idx)
{
    ...
    //auto contact_method = idx.data(static_cast<int>(Media::TextRecording::Role::ContactMethod)).value<ContactMethod*>();
    //auto contact_method_photo = contact_method->roleData((int) Call::Role::Photo);
    //auto sender_image = (QString) GlobalInstances::pixmapManipulator().toByteArray(contact_method_photo).toBase64();
    ...
    //message_object.insert("sender_image", QJsonValue(sender_image));
    message_object.insert("timestamp", QJsonValue((int) timestamp));
    message_object.insert("direction", QJsonValue((direction == Media::Media::Direction::IN) ? QString("in") : QString("out")));
    ...
    switch(idx.data(static_cast<int>(Media::TextRecording::Role::DeliveryStatus)).value<Media::TextRecording::Status>())
    {
        ...
        {
            message_object.insert("delivery_status", QJsonValue(QString("failure")));
            break;
        }
        case Media::TextRecording::Status::COUNT__:
        {
            message_object.insert("delivery_status", QJsonValue(QString("count__")));
            break;
        }
        case Media::TextRecording::Status::SENDING:
        {
            message_object.insert("delivery_status", QJsonValue(QString("sending")));
            break;
        }
        case Media::TextRecording::Status::UNKNOWN:
        {
            message_object.insert("delivery_status", QJsonValue(QString("unknown")));
            break;
        }
        case Media::TextRecording::Status::READ:
        {
            message_object.insert("delivery_status", QJsonValue(QString("read")));
            break;
        }
        case Media::TextRecording::Status::SENT:
        {
            message_object.insert("delivery_status", QJsonValue(QString("sent")));
            break;
        }
    }
...
print_message_to_buffer(ChatView* self, const QModelIndex &idx)
{
    ...
    g_warning("adding message: %s", function_call);
...
static void set_participant_images(ChatView* self)
{
    ChatViewPrivate *priv = CHAT_VIEW_GET_PRIVATE(self);

    //TODO: set sender image for "ME"

    /* Find the peer's name and picture */
    QString sender_name;
    QVariant photo_variant;

    if (priv->person)
    {
        photo_variant = priv->person->photo();
        sender_name = priv->person->roleData(static_cast<int>(Ring::Role::Name)).toString();
    }
    else
    {
        ...
        sender_name = contact_method->roleData(static_cast<int>(Ring::Role::Name)).toString();
        photo_variant = contact_method->roleData((int) Call::Role::Photo);
    }
    QString sender_image = (QString) GlobalInstances::pixmapManipulator().toByteArray(photo_variant).toBase64();

    /* Create the set_sender_image_object */
    QJsonObject message_object = QJsonObject();
    message_object.insert("sender", QJsonValue(sender_name));
    message_object.insert("sender_image", QJsonValue(sender_image));
    auto set_sender_image_object = QString(QJsonDocument(message_object).toJson(QJsonDocument::Compact)).toUtf8().constData();

    /* Call setSenderImage */
    gchar* function_call = g_strdup_printf("ring.chatview.setSenderImage(%s);", set_sender_image_object);
    g_warning("setting sender image: %s", function_call);
    webkit_web_view_run_javascript(
        WEBKIT_WEB_VIEW(priv->webview_chat),
        function_call,
        NULL,
...
load_javascript_libs(WebKitWebView *webview_chat,
                     ChatView* self)
{
    g_warning("Starting to load javascript libraries");
    ChatViewPrivate *priv = CHAT_VIEW_GET_PRIVATE(self);
