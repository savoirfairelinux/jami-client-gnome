...
message_index_to_json_message_object(const QModelIndex &idx)
{
    ...
    auto message_id = idx.data(static_cast<int>(Media::TextRecording::Role::Id)).value<uint64_t>();

    QJsonObject message_object = QJsonObject();
    message_object.insert("text", QJsonValue(message));
    message_object.insert("id", QJsonValue((qint64) message_id));
...
print_text_recording(Media::TextRecording *recording, ChatView *self)
{
    ...
    priv->message_changed_connection = QObject::connect(
        ...
        [self, priv] (const QModelIndex & topLeft, G_GNUC_UNUSED const QModelIndex & bottomRight)
        {
            ...
            g_warning("updating message: %s", function_call);
            webkit_web_view_run_javascript(
                WEBKIT_WEB_VIEW(priv->webview_chat),
                function_call,
                NULL,
                          ...
                          ChatView* self)
{
    ...
    g_warning("Loaded javascript library: %s", (const gchar*) loaded_library->data);
    ...
    //REVIEW: do I have to fee the string? how?
                     ...
                     ChatView* self)
{
    ...
    /* Create the array of libraries to load */
