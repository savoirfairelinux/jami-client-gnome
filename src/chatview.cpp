...
update_if_banned(ChatView *self)
{
    ...
    try {
        ...
        webkit_chat_enable_banned(WEBKIT_CHAT_CONTAINER(priv->webkit_chat_container), contactInfo.isBanned);
    } catch (const std::out_of_range&) {
        // ContactModel::getContact() exception
    }
}
...
update_chatview_frame(ChatView* self)
{
    ...
    bool invited;
    invited = contactInfo.profileInfo.type == lrc::api::profile::Type::TEMPORARY || contactInfo.profileInfo.type == lrc::api::profile::Type::PENDING;

    webkit_chat_update_chatview_frame(WEBKIT_CHAT_CONTAINER(priv->webkit_chat_container), contactInfo.isBanned, invited, alias.c_str(), bestName.c_str());
    g_debug("I got %s, %s, %s, %s", contactInfo.isBanned ? "true" : "false", invited ? "true" : "false", alias.c_str(), bestName.c_str());
}
