...
webkit_chat_container_script_dialog(G_GNUC_UNUSED GtkWidget* webview, gchar *interaction, ChatView* self)
{
    ...
    if (order == "ACCEPT") {
        ...
        // send file if any
        if (priv->filename) {
            g_warning("filename: %s", priv->filename);
            priv->accountContainer_->info.conversationModel->sendMessage(priv->conversation_->uid, std::string("file attached : " + std::string(priv->filename)));
            priv->filename = nullptr;
        }
        
    } else if (order == "ADD_FILE:") {
