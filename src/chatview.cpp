...
chat_view_dispose(GObject *object)
{
    ...
    if (priv->webkit_chat_container) {
        ...
        g_signal_handler_disconnect(priv->webkit_chat_container, priv->webkit_crashed);
        priv->webkit_crashed = 0;
...
update_if_banned(ChatView *self)
{
    ...
    try {
        ...
        if (contactInfo.isBanned) {
            g_debug("Contact is banned, display stuff");
...
static void
webkit_crashed(ChatView* self)
{
    g_debug("ChatView::webkit_crashed re-initing view");
...
build_chat_view(ChatView* self)
{
    ...
    priv->webkit_crashed = g_signal_connect_swapped(priv->webkit_chat_container,
        "web-process-crashed",
        G_CALLBACK(webkit_crashed),
        self);
