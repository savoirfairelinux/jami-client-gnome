...
button_send_invitation_clicked(ChatView *self)
{
    ...
    if (!account) {
        // try to use a selected account
        if (!AccountModel::instance().selectedAccount()) {
            g_warning("invalid account, cannot send invitation!");
            return;
        }

        // set the selected account to the cm
        account = AccountModel::instance().selectedAccount();
        priv->cm->setAccount(account);
    }

    // get a certificate
    auto certificate = CertificateModel::instance().getCertificateFromPath("certificate.test", account); // XXX
    certificate->setContactMethod(cm);

    if (!certificate) {
        g_warning("invalid certificate, cannot send invitation!");
        return;
    }

    if (!account->requestTrust(certificate))
        g_warning("trust request not forwarded, cannot send invitation!");
