...
static void
contact_popover_init(G_GNUC_UNUSED ContactPopover *self)
{
}
...
{
    g_debug("contact popover destroyed");
...
#if GTK_CHECK_VERSION(3,12,0)
static void
contact_popover_destroy(GtkPopover *popover)
{
...
contact_popover_class_init(ContactPopoverClass *klass)
{
    ...
    GTK_POPOVER_CLASS(klass)->closed = contact_popover_destroy;
#else
    /* destroy the window on ESC, or when the user clicks outside of it */
    GTK_WIDGET_CLASS(klass)->button_release_event = contact_popover_button_release;
    GTK_WIDGET_CLASS(klass)->key_press_event = contact_popover_key_press;
#endif
}
...
construct_edit_contact_view(ContactPopover *self, Person *p)
{
    ...
    g_signal_connect_swapped(priv->editcontactview, "person-saved", G_CALLBACK(contact_popover_destroy), self);
}
...
GtkWidget *
contact_popover_new(ContactMethod *cm, GtkWidget *parent, GdkRectangle *rect)
{
    ...
    gpointer self = g_object_new(EDIT_CONTACT_VIEW_TYPE,
                                 "transient-for", parent,
                                 "window-postion", GTK_WIN_POS_CENTER_ON_PARENT,
                                 NULL);
#endif

    // g_object_set(G_OBJECT(self), "height-request", 400, "width-request", 300, NULL);
