...
export_on_the_ring_clicked(G_GNUC_UNUSED GtkButton *button, AccountDevicesTab *view)
{
    ...
    {
        QObject::disconnect(priv->export_on_ring_ended);
        gtk_label_set_text(GTK_LABEL(priv->label_export_on_ring_error), _("Could not initiate exportOnRing, try again"));
...
static void
button_export_on_ring_error_ok_clicked(G_GNUC_UNUSED GtkButton *button, AccountDevicesTab *view)
{
    show_add_device_view(view);
}
...
build_tab_view(AccountDevicesTab *view)
{
    ...
    g_signal_connect(priv->button_export_on_ring_error_ok, "clicked", G_CALLBACK(button_export_on_ring_error_ok_clicked), view);
    ...
    auto *devices_model = gtk_q_tree_model_new(
        ...
        2,
        0, RingDevice::Role::Id, G_TYPE_STRING,
        0, RingDevice::Role::Name, G_TYPE_STRING);
