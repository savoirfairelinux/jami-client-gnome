...
static void person_photo_callback(G_GNUC_UNUSED GObject *source_object,
                                  GAsyncResult *res,
                                  gpointer user_data)
{
    GError *error = NULL;
    GdkPixbuf *pixbuf = gdk_pixbuf_new_from_stream_finish(res, &error);
...
GdkPixbuf *pxbm_person_photo(const QByteArray& data)
{
    ...
    // pixbuf = gdk_pixbuf_new_from_stream_async(stream, NULL, person_photo_callback, &error);
                                   ...
                                   bool displayInformation)
{
    ...
    return pxbm_scale_and_frame(temporary_item_avatar(), size, displayInformation);
