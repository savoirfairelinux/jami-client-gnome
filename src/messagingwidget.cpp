...
public:
    ...
    // timer related
    std::chrono::seconds timeElapsed {0};
...
static void on_leave_widget(MessagingWidget *self);
...
set_state(MessagingWidget *self, MessagingWidgetState state)
{
    ...
    case MESSAGING_WIDGET_REC_AUDIO:
    {
        ...
        g_timeout_add_seconds(1, on_timer_update, self);
        gtk_label_set_text(GTK_LABEL(priv->label_duration), "00:00");
        gtk_widget_show(priv->label_duration);

        priv->cpp->save_file_name = save_file_name;
    ...
    case MESSAGING_WIDGET_AUDIO_REC_SUCCESS:
        if (!priv->cpp->save_file_name.empty()) {
...
on_timer_update(gpointer user_data)
{
    ...
    if (priv->cpp->state != MESSAGING_WIDGET_REC_AUDIO) {
        // state changed, stop timer
        ret = FALSE;
    } else if (priv->cpp->timeElapsed == priv->cpp->timeLimit) {
        // time limit reached, stop timer and change state
        set_state(self, MESSAGING_WIDGET_AUDIO_REC_SUCCESS);
        ret = FALSE;
    } else {
        priv->cpp->timeElapsed++;
        gchar* formatted = g_strdup_printf("00:%i", priv->cpp->timeElapsed.count());
        gtk_label_set_text(GTK_LABEL(priv->label_duration), formatted);
        g_free(formatted);
    }

    if (!ret) {
        // reset timer value before stopping it
        priv->cpp->timeElapsed = std::chrono::seconds(0);
    }
...
on_record_button_pressed(MessagingWidget *self)
{
    ...
    case MESSAGING_WIDGET_REC_AUDIO:
        ...
        break;
    case MESSAGING_WIDGET_AUDIO_REC_SUCCESS:
        set_state(self, MESSAGING_WIDGET_REC_SENT);
        break;
    default:
        set_state(self, MESSAGING_WIDGET_STATE_INIT);
