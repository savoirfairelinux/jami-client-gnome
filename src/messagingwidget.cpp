...
on_timer_update(gpointer user_data)
{
    ...
    if (!ret) {
        ...
        priv->cpp->timeElapsed_ = std::chrono::seconds(0);
    }
...
CppImpl::set_state(MessagingWidgetState state)
{
    ...
    switch (state) {
    case MESSAGING_WIDGET_STATE_INIT:
        gtk_label_set_text(GTK_LABEL(widgets->label_letmessage), _("Oops, contact is busy. Want to let a message ?"));
    ...
    case MESSAGING_WIDGET_REC_AUDIO:
    {
        ...
        gtk_label_set_text(GTK_LABEL(widgets->label_duration), "00:00");
    ...
    case MESSAGING_WIDGET_REC_SENT:
        if (auto model = (*accountInfo_)->conversationModel.get()) {
            model->sendFile(conversation_->uid, save_file_name_, g_path_get_basename(save_file_name_.c_str()));
            save_file_name_ = "";
        }
