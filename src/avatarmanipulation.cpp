    ...
    if (res == GTK_RESPONSE_ACCEPT) {
        if(auto filename = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER (dialog))) {
            ...
            if (!error) {
                ...
                priv->width = gdk_pixbuf_get_width(priv->pix_scaled);
                priv->height = gdk_pixbuf_get_height(priv->pix_scaled);
            } else {
...
update_preview_cb(GtkFileChooser *file_chooser, GtkWidget *preview)
{
    ...
    if (auto filename = gtk_file_chooser_get_preview_filename(file_chooser)) {
        ...
        if (!error) {
            ...
            g_warning("(update_preview_cb) could not get pixbuf from file: %s", error->message);
        }
        g_free (filename);
    }
