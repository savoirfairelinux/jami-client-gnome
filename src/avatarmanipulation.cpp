...
struct _AvatarManipulationPrivate
{
    ...
    GtkWidget *button_cancel_avatar;
...
static void take_a_photo(G_GNUC_UNUSED GtkButton *button, AvatarManipulation *win);
static void choose_picture(G_GNUC_UNUSED GtkButton *button, AvatarManipulation *win);
static void export_avatar(G_GNUC_UNUSED GtkButton *button, G_GNUC_UNUSED AvatarManipulation *win);
static void update_preview_cb(GtkFileChooser *file_chooser, gpointer data);
static void cancel_photo(G_GNUC_UNUSED GtkButton *button, AvatarManipulation *win);
static void set_avatar(G_GNUC_UNUSED GtkButton *button, AvatarManipulation *win);
static void got_snapshot(G_GNUC_UNUSED VideoWidget *self, AvatarManipulation *win);

/* area selected */
static gboolean selector_widget_button_press_event(G_GNUC_UNUSED GtkWidget *widget, GdkEventButton *event
                                                                                             , AvatarManipulation *win);
...
static gboolean selector_widget_configure_event(G_GNUC_UNUSED GtkWidget *widget, G_GNUC_UNUSED GdkEventConfigure *event
                                                                                             , AvatarManipulation *win);
static gboolean selector_widget_draw(G_GNUC_UNUSED GtkWidget *widget, cairo_t *cr, AvatarManipulation *win);
static void update_and_draw(gdouble x, gdouble y, AvatarManipulation *win);
static void rescale(gdouble x, gdouble y, AvatarManipulation *win);
...
avatar_manipulation_class_init(AvatarManipulationClass *klass)
{
    ...
    gtk_widget_class_bind_template_child_private(GTK_WIDGET_CLASS (klass), AvatarManipulation, button_cancel_avatar);
...
static void
avatar_manipulation_init(AvatarManipulation *win)
{
    AvatarManipulationPrivate *priv = AVATAR_MANIPULATION_GET_PRIVATE(win);
    ...
    g_signal_connect(priv->selector_widget, "draw", G_CALLBACK (selector_widget_draw), win);
    g_signal_connect(priv->selector_widget,"configure-event", G_CALLBACK (selector_widget_configure_event), win);
    /* Event signals */
    g_signal_connect(priv->selector_widget, "motion-notify-event", G_CALLBACK (selector_widget_motion_notify_event)
                                                                                                                 , win);
    g_signal_connect(priv->selector_widget, "button-press-event", G_CALLBACK (selector_widget_button_press_event), win);
    ...
    g_signal_connect(priv->video_widget, "snapshot-taken", G_CALLBACK (got_snapshot), win);
    ...
    /* signals */
    g_signal_connect(priv->button_choose_picture, "clicked", G_CALLBACK(choose_picture), win);
    g_signal_connect(priv->button_take_photo, "clicked", G_CALLBACK(take_a_photo), win);
    g_signal_connect(priv->button_set_avatar, "clicked", G_CALLBACK(set_avatar), win);
    g_signal_connect(priv->button_cancel_avatar, "clicked", G_CALLBACK(cancel_photo), win);
    g_signal_connect(priv->button_export_avatar, "clicked", G_CALLBACK(export_avatar), win);
...
static void
take_a_photo(G_GNUC_UNUSED GtkButton *button, AvatarManipulation *win)
{
    AvatarManipulationPrivate *priv = AVATAR_MANIPULATION_GET_PRIVATE(win);
...
static void
cancel_photo(G_GNUC_UNUSED GtkButton *button, AvatarManipulation *win)
{
    Video::PreviewManager::instance().startPreview();
    AvatarManipulationPrivate *priv = AVATAR_MANIPULATION_GET_PRIVATE(win);
    ...
    gtk_widget_set_visible (priv->button_cancel_avatar, false);
...
static void
set_avatar(G_GNUC_UNUSED GtkButton *button, AvatarManipulation *win)
{
    Video::PreviewManager::instance().stopPreview();
    AvatarManipulationPrivate *priv = AVATAR_MANIPULATION_GET_PRIVATE(win);
    ...
    /* show the avatar */
    gtk_image_set_from_pixbuf(GTK_IMAGE(priv->image_avatar), pixbuf_frame_resized);
    ...
    gtk_widget_set_visible (priv->button_set_avatar, false);
    gtk_widget_set_visible (priv->button_cancel_avatar, true);
...
static void
choose_picture(G_GNUC_UNUSED GtkButton *button, AvatarManipulation *win)
{
    AvatarManipulationPrivate *priv = AVATAR_MANIPULATION_GET_PRIVATE(win);
    ...
    GtkWidget *ring_main_window = gtk_widget_get_toplevel(GTK_WIDGET(win));
...
static void
export_avatar(G_GNUC_UNUSED GtkButton *button, G_GNUC_UNUSED AvatarManipulation *win)
{
...
static gboolean
selector_widget_draw(G_GNUC_UNUSED GtkWidget *widget, cairo_t *cr, AvatarManipulation *win)
{
    AvatarManipulationPrivate *priv = AVATAR_MANIPULATION_GET_PRIVATE(win);
...
static gboolean
selector_widget_motion_notify_event(G_GNUC_UNUSED GtkWidget *widget, GdkEventMotion *event, AvatarManipulation *win)
{
    AvatarManipulationPrivate *priv = AVATAR_MANIPULATION_GET_PRIVATE(win);
    ...
    if (priv->do_action && state == GDK_BUTTON1_MASK) {
        switch (priv->action_on_selector) {
        case MOVE_SELECTOR:
            update_and_draw( x - priv->relative_position[0], y - priv->relative_position[1] , win );
        break;
        case PICK_SELECTOR:
            rescale( x, y , win );
        break;
...
static gboolean
selector_widget_button_press_event(G_GNUC_UNUSED GtkWidget *widget, GdkEventButton *event, AvatarManipulation *win)
{
    AvatarManipulationPrivate *priv = AVATAR_MANIPULATION_GET_PRIVATE(win);
...
static gboolean
selector_widget_button_release_event(G_GNUC_UNUSED GtkWidget *widget, GdkEventButton *event, AvatarManipulation *win)
{
    AvatarManipulationPrivate *priv = AVATAR_MANIPULATION_GET_PRIVATE(win);
    ...
    if (priv->do_action)
        ...
        break;
        case PICK_SELECTOR:
            update_and_draw( priv->origin[0], priv->origin[1], win );
        break;
        }
...
static void
update_and_draw(gdouble x, gdouble y, AvatarManipulation *win)
{
    AvatarManipulationPrivate *priv = AVATAR_MANIPULATION_GET_PRIVATE(win);
...
static void
rescale(gdouble x, gdouble y, AvatarManipulation *win)
{
    AvatarManipulationPrivate *priv = AVATAR_MANIPULATION_GET_PRIVATE(win);
...
static void
got_snapshot(G_GNUC_UNUSED VideoWidget *self, AvatarManipulation *win)
{
    AvatarManipulationPrivate *priv = AVATAR_MANIPULATION_GET_PRIVATE(win);
