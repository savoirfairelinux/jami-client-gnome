...
static void cancel_photo(G_GNUC_UNUSED GtkButton *button, AvatarManipulation *self);
...
avatar_manipulation_init(AvatarManipulation *self)
{
    ...
    g_signal_connect(priv->button_cancel_avatar, "clicked", G_CALLBACK(cancel_photo), self);
...
static void
cancel_photo(G_GNUC_UNUSED GtkButton *button, AvatarManipulation *self)
{
    Video::PreviewManager::instance().startPreview();
    AvatarManipulationPrivate *priv = AVATAR_MANIPULATION_GET_PRIVATE(self);
...
activate_take_a_photo(AvatarManipulation *self)
{
     ...
     * isPreviewing() will returns true, but the preview will be black. So far, there is now to get back the preview.
     * if some other application is already using the webcam, isPreviewing() returns true (bug?) meanwhile previewRenderer()
     * return false.
     */
    if(Video::PreviewManager::instance().previewRenderer()) {
        gtk_widget_set_sensitive(priv->button_take_photo,true);
        gtk_widget_set_visible(priv->video_widget,true);
    } else {
        gtk_widget_set_sensitive(priv->button_take_photo,false);
        gtk_widget_set_visible(priv->video_widget, false);
        gtk_widget_set_visible(priv->image_avatar, true);
        gtk_stack_set_visible_child_name(GTK_STACK(priv->stack_views), "page_avatar");
    }
}
