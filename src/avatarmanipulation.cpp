...
static void take_a_photo(GtkButton *button, AvatarManipulation *self);
static void choose_picture(GtkButton *button, AvatarManipulation *self);
static void export_avatar(GtkButton *button, AvatarManipulation *self);
static void update_preview_cb(GtkFileChooser *file_chooser, GtkWidget *preview);
static void trash_photo(GtkButton *button, AvatarManipulation *self);
static void set_avatar(GtkButton *button, AvatarManipulation *self);
static void got_snapshot(VideoWidget *self, AvatarManipulation *parent);
...
avatar_manipulation_dispose(GObject *object)
{
    ...
    if (ProfileModel::instance().selectedProfile()) {
        ...
        /* useful if the user hits "Next" without any avatar at the creation of his profil */
        if (not photo.isValid() && priv->video_widget) {
...
set_avatar(G_GNUC_UNUSED GtkButton *button, AvatarManipulation *self)
{
    ...
    if (!png_buffer_signed) {
        g_message("(set_avatar) function, error message: %s\n", error->message);
        g_error_free(error);
        return;
    }
...
choose_picture(G_GNUC_UNUSED GtkButton *button, AvatarManipulation *self)
{
    ...
    if (res == GTK_RESPONSE_ACCEPT) {
        ...
        if(filename) {
            ...
            g_message("(choose_picture) error message: %s\n", error->message);
            g_error_free(error);
        }
...
export_avatar(G_GNUC_UNUSED GtkButton *button, G_GNUC_UNUSED AvatarManipulation *self)
{
    ...
    dialog = gtk_file_chooser_dialog_new (_("Save Avatar Image"),
                                          GTK_WINDOW(ring_main_window),
                                          action,
                                          _("_Cancel"),
                                          GTK_RESPONSE_CANCEL,
    ...
    if (res == GTK_RESPONSE_ACCEPT) {
        ...
        filename = gtk_file_chooser_get_filename(chooser);
        if (ProfileModel::instance().selectedProfile() && filename) {
            GError* error =  nullptr;
            auto photo = ProfileModel::instance().selectedProfile()->person()->photo();
            ...
            if (photo.isValid()) {
                ...
                if (error){
                    g_message("(export_avatar) error message: %s\n", error->message);
                    g_error_free(error);
                }
            }
...
update_preview_cb(GtkFileChooser *file_chooser, GtkWidget *preview)
{
    ...
    if (filename) {
        GError* error =  nullptr;
        pixbuf = gdk_pixbuf_new_from_file_at_size (filename, 128, 128, &error);
        if (error) {
            g_message("(update_preview_cb) error message: %s\n", error->message);
        }
        have_preview = (pixbuf != NULL);
        g_free (filename);
    } else {
        g_message("(update_preview_cb) error message: filename = nullptr\n");
    }
