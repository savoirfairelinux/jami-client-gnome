...
ring_notifier_class_init(RingNotifierClass *klass)
{
    ...
    ring_notifier_signals[TRIGGER_ACTION] = g_signal_new(
        "clicked",
...
#if USE_LIBNOTIFY
static void
ring_notify_trigger_action(NotifyNotification*, char* id, RingNotifier* view)
{
    g_signal_emit(G_OBJECT(view), ring_notifier_signals[TRIGGER_ACTION], 0, id);
}
...
ring_show_notification(RingNotifier* view, const std::string& id, const std::string& icon, const std::string& title, const std::string& body, bool urgent, bool playSound)
{
    ...
    if (priv->serverInfo.actions) {
        notify_notification_add_action(notification.get(),
                                       id.c_str(),
                                       C_("", "Open"),
                                       (NotifyActionCallback)ring_notify_trigger_action,
                                       view,
                                       nullptr);
    }
