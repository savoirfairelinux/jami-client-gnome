                       ...
                       const std::string& body, NotificationType type)
{
    ...
    // Draw icon
    auto nameStr = name.empty() ? "" : QString(QString(name.c_str()).at(0)).toStdString();  // NOTE best way to be compatible with UTF-8
    auto default_avatar = Interfaces::PixbufManipulator().generateAvatar(name, uri);
