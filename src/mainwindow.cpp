...
CppImpl::updateUrgency()
{
    ...
    g_warning("notif count is %d", lrc_->getAccountModel().notificationsCount());
    auto* priv = MAIN_WINDOW_GET_PRIVATE(MAIN_WINDOW(self));
...
{
    g_warning("%s", "from slotInteractionStatusUpdated");
    updateUrgency();
}
void
CppImpl::slotConversationUpdated(const std::string& uid)
{
    g_warning("%s", "from slotConversationUpdated");
    updateUrgency();
}
