...
public:
    ...
    void updateUrgency(bool force_urgent = false);
...
action_notification(gchar* title, MainWindow* self, Action action)
{
    ...
    if (action == Action::SELECT) {
        ...
        if (type == "interaction") {
            ...
            for (const lrc::api::conversation::Info& conversation : priv->cpp->accountInfo_->conversationModel->getFilteredConversations(lrc::api::profile::Type::PENDING).get()) {
...
void
CppImpl::changeView(GType type, OptRef<lrc::api::conversation::Info> convOpt)
{
    leaveFullScreen();
    ...
    if (convOpt)
    {
        ...
        if (g_type_is_a(INCOMING_CALL_VIEW_TYPE, type)) {
            new_view = displayIncomingView(conv, redraw_webview);
        } else if (g_type_is_a(CURRENT_CALL_VIEW_TYPE, type)) {
            new_view = displayCurrentCallView(conv, redraw_webview);
        } else if (g_type_is_a(CHAT_VIEW_TYPE, type)) {
            new_view = displayChatView(conv, redraw_webview);
        } else {
            g_warning("Ignoring conversation");
            new_view = displayWelcomeView();
        }

        if (conv.uid.isEmpty() && type != WELCOME_VIEW_TYPE)
            conversations_view_select_conversation(
                    CONVERSATIONS_VIEW(widgets->treeview_conversations),
                    conv.uid.toStdString());
    } else {
        if (type != WELCOME_VIEW_TYPE)
            g_warning("Invalid conversation");
...
void
CppImpl::updateUrgency(bool force_urgent)
{
    auto urgent = force_urgent;
    foreachLrcAccount(*lrc_, [&] (const auto& acc_info) {
        if (!urgent) {
            auto conversations =
              acc_info.conversationModel->allFilteredConversations();
            for (const auto& conversation: conversations.get()) {
                if (conversation.get().unreadMessages) {
                    urgent = true;
                    break;
                }
            }
            if (!urgent) {
                urgent = acc_info.contactModel->hasPendingRequests();
            }
        }
    });
    auto* priv = MAIN_WINDOW_GET_PRIVATE(MAIN_WINDOW(self));
    if (urgent != priv->is_urgent) {
        g_signal_emit(G_OBJECT(self),
                      main_window_signals[URGENCY_CHANGED], 0,
                      urgent);
        gtk_window_set_urgency_hint(GTK_WINDOW(self), urgent);
        priv->is_urgent = urgent;
    }
}
...
CppImpl::updateLrc(const std::string& id, const std::string& accountIdToFlagFreeable)
{
    ...
    interactionStatusUpdatedConnection_ =
        QObject::connect(&*accountInfo_->conversationModel,
                         ...
                         [this] (const QString& convUid, uint64_t msgId,
...
CppImpl::slotNewIncomingCall(const std::string& accountId, const std::string& convUid)
{
    ...
    if (!accountInfo.callModel->hasCall(conv.callId)) {
        g_warning("Incoming call %s not found", conv.callId.toStdString().c_str());
        return;
    }

    auto call = accountInfo.callModel->getCall(conv.callId);
    ...
    if (call.status == lrc::api::call::Status::INCOMING_RINGING) {
        ...
        try {
            ...
            notifId = accountInfo.id.toStdString() + ":call:" + conv.callId.toStdString();
        } catch (...) {
...
CppImpl::slotShowCallView(const std::string& id, const std::string& convId)
{
    ...
    if (IS_CURRENT_CALL_VIEW(old_view)
        && conv.uid == current_call_view_get_conversation(CURRENT_CALL_VIEW(old_view)).uid)
    {
        auto rendered = QString::fromStdString(current_call_view_get_rendered_call(CURRENT_CALL_VIEW(old_view)));
        auto sameCall = conv.confId.isEmpty() ?
            conv.callId == rendered :
            conv.confId == rendered;
        if (sameCall)
            return;
    }
...
main_window_class_init(MainWindowClass *klass)
{
    ...
    /* add signals */
    main_window_signals[URGENCY_CHANGED] =
        g_signal_new("main-window-urgency-changed",
