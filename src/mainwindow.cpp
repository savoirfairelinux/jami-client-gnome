...
CppImpl::changeView(GType type, OptRef<lrc::api::conversation::Info> convOpt)
{
    ...
    if (g_type_is_a(INCOMING_CALL_VIEW_TYPE, type)) {
        new_view = displayIncomingView(convOpt.value(), redraw_webview);
    } else if (g_type_is_a(CURRENT_CALL_VIEW_TYPE, type)) {
        new_view = displayCurrentCallView(convOpt.value(), redraw_webview);
    } else if (g_type_is_a(CHAT_VIEW_TYPE, type)) {
        new_view = displayChatView(convOpt.value(), redraw_webview);
    } else {
    ...
    if (convOpt && convOpt->get().uid != "" && type != WELCOME_VIEW_TYPE)
