...
public:
    ...
    void add_conference(const std::vector<std::string>& uris, const std::string& custom_data);
                    ...
                    QObject::connect(&*(*priv->cpp->accountInfo)->callModel, &lrc::api::NewCallModel::callStatusChanged,
                        [priv, confCallId, callToRender](const std::string& callId, int code) {
                            ...
                            case Status::IN_PROGRESS:
                            {
                                ...
                                (*priv->cpp->accountInfo)->callModel->joinCalls(callToRender, confCallId);
                                return;
                            }
                            default:
               ...
               lrc::api::AVModel& avModel)
{
    ...
    if ((*accountInfo)->profileInfo.type == lrc::api::profile::Type::RING) {
        ...
        for (const auto& c : lrc_.getConferences()) {
            ...
            if (!uri.empty()) add_conference(curis, c);
        }
...
void
CppImpl::add_conference(const std::vector<std::string>& uris, const std::string& custom_data)
{
