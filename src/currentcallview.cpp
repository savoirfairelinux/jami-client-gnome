...
update_details(CurrentCallView *view)
{
    ...
    auto callRendered = priv->conversation_->info.callId;

    if (!priv->conversation_->info.confId.empty())
        callRendered = priv->conversation_->info.confId;
...
{
    auto priv = CURRENT_CALL_VIEW_GET_PRIVATE(view);
    auto callToHangUp = priv->conversation_->info.callId;
    if (!priv->conversation_->info.confId.empty())
        callToHangUp = priv->conversation_->info.confId;
...
{
    auto priv = CURRENT_CALL_VIEW_GET_PRIVATE(view);
    auto callToHold = priv->conversation_->info.callId;
    if (!priv->conversation_->info.confId.empty())
        callToHold = priv->conversation_->info.confId;
...
{
    auto priv = CURRENT_CALL_VIEW_GET_PRIVATE(view);
    auto callToRecord = priv->conversation_->info.callId;
    if (!priv->conversation_->info.confId.empty())
        callToRecord = priv->conversation_->info.confId;
...
{
    auto priv = CURRENT_CALL_VIEW_GET_PRIVATE(view);
    auto callToMute = priv->conversation_->info.callId;
    if (!priv->conversation_->info.confId.empty())
        callToMute = priv->conversation_->info.confId;
