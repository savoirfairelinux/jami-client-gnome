...
static void
button_hold_clicked(CurrentCallView *view)
{
    auto priv = CURRENT_CALL_VIEW_GET_PRIVATE(view);
...
insert_controls(CurrentCallView *view)
{
    ...
    g_signal_connect_swapped(priv->button_hold, "clicked", G_CALLBACK(button_hold_clicked), view);
...
current_call_view_class_init(CurrentCallViewClass *klass)
{
    ...
    gtk_widget_class_bind_template_child_private(GTK_WIDGET_CLASS (klass), CurrentCallView, button_hold);
...
update_state(CurrentCallView *view)
{
    ...
    try {
        ...
        auto pauseBtn = GTK_TOGGLE_BUTTON(priv->button_hold);
