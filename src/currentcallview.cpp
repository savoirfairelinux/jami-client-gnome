...
public:
    ...
    void add_call(const std::vector<std::string>& uris);
    void add_present_contact(const std::string& uri);
        ...
        for (const auto& c : (*accountInfo)->conversationModel->getFilteredConversations(lrc::api::profile::Type::RING)) {
            try {
                ...
                if (contactInfo.isPresent && !isPresent) {
                    add_present_contact(participant);
                }
            } catch (...) {
                // TODO (sblin)
            }
...
void
CppImpl::add_call(const std::vector<std::string>& uris)
{
    if (uris.empty()) return;
    auto uri = uris.front();
    add_present_contact(uri);
}

void
CppImpl::add_present_contact(const std::string& uri)
{
    auto bestName = uri;
