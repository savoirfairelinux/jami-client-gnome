               ...
               lrc::api::conversation::Info* conv_info)
{
    ...
    if ((*accountInfo)->profileInfo.type == lrc::api::profile::Type::RING)
        ...
        auto* image_buf = ring_draw_fallback_avatar(48, "", 10);
        auto scaled = Interfaces::PixbufManipulator().scaleAndFrame(image_buf, QSize(48, 48));
        ...
        for (const auto& c : (*accountInfo)->conversationModel->getFilteredConversations(lrc::api::profile::Type::SIP)) {
            auto* box_item = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
            std::string tel = "sip:" + c.participants.front();
            auto* md5 = g_compute_checksum_for_string(G_CHECKSUM_MD5, tel.c_str(), -1);
            auto color = std::string("0123456789abcdef").find(md5[0]);
            auto* image_buf = ring_draw_fallback_avatar(48, "", color);
            auto scaled = Interfaces::PixbufManipulator().scaleAndFrame(image_buf, QSize(48, 48));
            auto* avatar = gtk_image_new_from_pixbuf(scaled.get());
