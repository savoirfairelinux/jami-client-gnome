...
set_call_quality(CurrentCallView* view, bool auto_quality_on, double desired_quality)
{
    ...
    for (const auto& codec : videoCodecs) {
        if (auto_quality_on) {
            ...
            int min_bitrate = 0, max_bitrate = 0, min_quality = 0, max_quality = 0;
            try {
            ...
            double bitrate;
            bitrate = min_bitrate + static_cast<double>(max_bitrate - min_bitrate)*(desired_quality/100.0);
            if (bitrate < 0) bitrate = 0;
            (*priv->cpp->accountInfo)->codecModel->bitrate(codec.id, bitrate);

            double quality;
            // note: a lower value means higher quality
            quality = static_cast<double>(min_quality) - (min_quality - max_quality)*(desired_quality/100.0);
