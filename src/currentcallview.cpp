...
update_smartInfo(CurrentCallView *view, Call *call)
{
    ...
    QObject::connect(&SmartInfoHub::instance(), &SmartInfoHub::changed,
        [priv] () {
            ...
            gtk_label_set_text(GTK_LABEL(priv->label_smartInfo),SmartInfoHub::instance().localVideoCodec().toStdString().c_str());
            if(SmartInfoHub::instance().localFps() != 0){
                gtk_label_set_text(GTK_LABEL(priv->label_smartInfo2),QString::number(SmartInfoHub::instance().localFps()).toStdString().c_str());
            }
