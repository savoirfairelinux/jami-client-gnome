...
public:
    ...
    void set_remote_record_animation(std::string callId, VectorString peerName, bool state);
...
CppImpl::setCallInfo()
{
    ...
    if (not call.peerRec.isEmpty()) {
        g_warning("@@@ At least one peast is recording");
        set_remote_record_animation(currentCall_, call.peerRec, true);
    } else
    {
        g_warning("@@@ no peer recording");
    }
    ...
    remoteinfo_connection = QObject::connect(
        ...
        [this] (const QString& callId, const QString& peerName, bool state) {
            g_return_val_if_fail(IS_CURRENT_CALL_VIEW(self), FALSE);
            auto priv = CURRENT_CALL_VIEW_GET_PRIVATE(self);

            set_remote_record_animation(callId.toStdString(), {peerName}, state);
        });
...
void
CppImpl::set_remote_record_animation(std::string callId, VectorString peerName, bool state)
{
    if (callId == currentCall_) {
        g_debug("@@@ callId == currentCall_: %s", callId.c_str());
        // Show control
        ...
        for (auto& uri: peerName) {
            try {
                uri.remove('\r');
                uri.remove('\n');
                label += uri.toStdString();
        ...
        if (state) {
            ...
            gchar* value = g_strdup_printf("%s is recording", label.c_str());
            ...
            gchar* value = g_strdup_printf("%s stopped recording", label.c_str());
            gtk_label_set_text(GTK_LABEL(widgets->label_remoteRecord_information),value);
            g_free(value);
            timer_record_fade = g_timeout_add(2000, (GSourceFunc)on_timer_record_fade_timeout, self);
        }
    } else {
        g_debug("@@@ callId != currentCall_");
    }
}
