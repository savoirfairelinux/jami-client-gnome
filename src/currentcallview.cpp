...
activate_media_handler(GtkListBox*, GtkListBoxRow* row, CurrentCallView* self)
{
    ...
    while (auto* children = gtk_list_box_get_row_at_index(GTK_LIST_BOX(priv->list_media_handlers_available), rowIdx)) {
        QString labelText = QString::fromStdString(gtk_label_get_text(label));
        gchar* text = nullptr;
        text = g_markup_printf_escaped(
            "<span font=\"10\">%s</span>",
            qUtf8Printable(_("Activate") + labelText)
            );
        if (children == row) {
...
static void
toggle_plugin(GtkSwitch* switchBtn, GParamSpec*, CurrentCallView *view)
{
    auto* priv = CURRENT_CALL_VIEW_GET_PRIVATE(view);
...
CppImpl::add_media_handler(lrc::api::plugin::MediaHandlerDetails mediaHandlerDetails)
{
    ...
    auto* load_plugin_button = gtk_switch_new();
    gtk_widget_set_can_focus(load_plugin_button, false);
    gtk_switch_set_active(GTK_SWITCH(load_plugin_button), isActive);
    g_signal_connect(load_plugin_button, "notify::active", G_CALLBACK(toggle_plugin), self);
    ...
    gtk_container_add(GTK_CONTAINER(box_item), GTK_WIDGET(load_plugin_button));
...
CppImpl::updatePluginList()
{
    ...
    gtk_list_box_set_selection_mode(GTK_LIST_BOX(widgets->list_media_handlers_available), GTK_SELECTION_NONE);
}
