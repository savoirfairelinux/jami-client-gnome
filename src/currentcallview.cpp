...
fullscreen_destroy(CurrentCallView *view)
{
    ...
    /* destroy fullscreen, start rendering in main window again */
    gtk_widget_destroy(priv->fullscreen_window);
    priv->fullscreen_window = NULL;
...
on_button_press_in_video_event(GtkWidget *self, GdkEventButton *event, CurrentCallView *view)
{
    ...
    if (event->type == GDK_2BUTTON_PRESS) {
        if (priv->fullscreen_window) {
            ...
            g_signal_connect_swapped(priv->fullscreen_window, "destroy", G_CALLBACK(fullscreen_destroy), view);
            g_signal_connect(priv->fullscreen_window, "key_press_event", G_CALLBACK(fullscreen_handle_keys), NULL);
