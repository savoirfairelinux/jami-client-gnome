...
struct _CurrentCallViewPrivate
{
     ...
     * to process any new renderers by stoping the g_idel function.
     */ 
...
current_call_view_init(CurrentCallView *view)
{
    ...
    /* new renderers don't happen often, but we want to display them as fast as possible,
     * timeout every 30 ms to check */
    priv->renderer_timeout_source = g_timeout_add(30, (GSourceFunc)check_renderer_queue, view);
}
