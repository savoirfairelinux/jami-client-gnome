...
struct _AccountImportExportViewPrivate
{
    ...
    GtkWidget *entry_export_location;
...
static void
export_file_set(AccountImportExportView *self)
{
...
password_changed(AccountImportExportView *self)
{
    ...
    if (strlen(password) > 0) {
        ...
        // export
        if (auto filename = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(priv->filechooserbutton_export))) {
...
export_account(AccountImportExportView *self)
{
    ...
    const auto filepath = gtk_entry_get_text(GTK_ENTRY(priv->entry_export_location));
    // validate filepath
    if (strlen(filepath)) {
        const auto password = gtk_entry_get_text(GTK_ENTRY(priv->entry_password));
        if (strlen(password) > 0) {
...
account_importexport_view_init(AccountImportExportView *self)
{
    ...
    g_signal_connect_swapped(priv->filechooserbutton_export, "file-set", G_CALLBACK(export_file_set), self);
...
account_importexport_view_class_init(AccountImportExportViewClass *klass)
{
    ...
    gtk_widget_class_bind_template_child_private(GTK_WIDGET_CLASS (klass), AccountImportExportView, entry_export_location);
...
account_export_view_set_accounts(AccountImportExportView *self, GList *accounts)
{
    ...
    if (accounts && accounts->data) {
        ...
        //TODO: update UI to indicate some accounts are selected
    } else {
        //TODO: update UI to indicate no accounts are selected
    }
}
