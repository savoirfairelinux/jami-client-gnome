    ...
    if (path_str_source && strlen(path_str_source) > 0) {
        ...
        if (gtk_tree_view_get_dest_row_at_pos(GTK_TREE_VIEW(treeview), x, y, &dest_path, NULL)) {
            ...
            if (idx_source.isValid() && idx_dest.isValid()) {
                auto call_source = CallModel::instance()->getCall(idx_source);
                auto call_dest = CallModel::instance()->getCall(idx_dest);

                if (call_source && call_dest && (call_source != call_dest)) {
                    g_debug("got our calls!");
                    success = TRUE;

                    CallModel::instance()->createConferenceFromCall(call_source, call_dest);
                } else if (call_source == call_dest) {
                    g_warning("tyring to create a conference between the same call");
                } else {
                    g_warning("one of the calls doesn't exist");
                }
            }
