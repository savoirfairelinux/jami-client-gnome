...
dring_crash_dialog()
{
    ...
    gtk_window_set_title(GTK_WINDOW(dialog), "Ring Error");
...
ring_quitting_dialog()
{
    ...
    GtkWidget *dialog = gtk_message_dialog_new(
        ...
        "Could not re-connect to the Ring daemon (dring); Ring will now quit."
    );
...
check_connection_cb(GtkWidget *warning_dialog)
{
    ...
    if ((!CallModel::instance()->isConnected()) || (!CallModel::instance()->isValid())) {
        ...
        static_cast<DBusErrorHandler*>(getDelegateManager()->getDBusErrorDelegate())->finishedHandlingError();
    }
