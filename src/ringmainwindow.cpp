                      ...
                      gpointer data)
{
    ...
    auto default_scaled = Interfaces::PixbufManipulator().scaleAndFrame(default_avatar.get(), QSize(36, 36), true, enabled);
    ...
    if (!photostr.empty()) {
        ...
        auto pixbuf_photo = Interfaces::PixbufManipulator().scaleAndFrame(avatar.value<std::shared_ptr<GdkPixbuf>>().get(), QSize(36, 36), true, enabled);
        if (avatar.isValid()) {
            photo = pixbuf_photo;
        }
    }
...
CppImpl::slotAccountAddedFromLrc(const std::string& id)
{
    ...
    try {
        ...
        if (!accountInfo_) {
            accountInfo_ = &lrc_->getAccountModel().getAccountInfo(id);
            auto* settings_view = gtk_stack_get_child_by_name(GTK_STACK(widgets->stack_main_view), NEW_ACCOUNT_SETTINGS_VIEW_NAME);
            settings_view = new_account_settings_view_new(accountInfo_);
        }
