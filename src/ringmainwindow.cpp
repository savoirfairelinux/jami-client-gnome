...
show_ring_id(RingMainWindow *win, Account *account) {
    ...
    if (account) {
        if (!account->username().isEmpty()) {
            const gchar *ring_id = account->username().toUtf8().constData();
            gtk_entry_set_text(GTK_ENTRY(priv->entry_ring_id), ring_id);
        } else {
...
ring_main_window_init(RingMainWindow *win)
{
    ...
    gtk_widget_set_size_request(priv->entry_ring_id, 350, 35);
