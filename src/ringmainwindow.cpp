...
static void
alias_entry_changed(GtkEditable *entry, G_GNUC_UNUSED gpointer data)
{
    const gchar *alias = gtk_entry_get_text(GTK_ENTRY(entry));
    if (!alias || strlen(alias) == 0) {
        /* username will be "Unknown" */
        gtk_entry_set_placeholder_text(GTK_ENTRY(entry), C_("This is the placeholder text if the user empties the username entry", "'Unknown' if empty..."));
    }
}
...
show_account_creation(RingMainWindow *win)
{
    ...
    /* connect signals */
    g_signal_connect(priv->entry_alias, "changed", G_CALLBACK(alias_entry_changed), nullptr);
