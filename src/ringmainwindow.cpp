...
public:
    ...
    std::shared_ptr<AccountContainer> accountContainer_;
...
on_dtmf_pressed(RingMainWindow* self, GdkEventKey* event, gpointer user_data)
{
    ...
    if (!priv->cpp->accountContainer_->isEmpty() &&
...
CppImpl::CppImpl(RingMainWindow& widget)
    ...
    , accountContainer_ {new AccountContainer()}
{}
...
CppImpl::displayIncomingView(lrc::api::conversation::Info conversation)
{
    ...
    return incoming_call_view_new(webkitChatContainer(), accountContainer_, chatViewConversation_.get());
}
...
CppImpl::displayCurrentCallView(lrc::api::conversation::Info conversation)
{
    ...
    auto* new_view = current_call_view_new(webkitChatContainer(),
                                           accountContainer_, chatViewConversation_.get());
...
{
    chatViewConversation_.reset(new lrc::api::conversation::Info(conversation));
    auto* new_view = chat_view_new(webkitChatContainer(), accountContainer_, chatViewConversation_.get());
...
CppImpl::updateLrc(const std::string& id, const std::string& accountIdToFlagFreeable)
{
    ...
    if (!id.empty())
        accountContainer_->updateInfo(lrc_->getAccountModel().getAccountInfo(id));
    else
        accountContainer_->resetInfo();
    ...
    if (accountContainer_->isEmpty()) return;
