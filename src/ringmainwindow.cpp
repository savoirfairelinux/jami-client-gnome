...
show_account_creation(RingMainWindow *win)
{
    ...
    gtk_box_pack_start(GTK_BOX(priv->account_creation), priv->button_choose_a_picture, TRUE, TRUE, 0);
    gtk_widget_show_all(priv->button_choose_a_picture);
...
choose_a_picture(G_GNUC_UNUSED GtkButton *button, RingMainWindow *win)
{
    ...
    if (res == GTK_RESPONSE_ACCEPT)
      {
        char *filename;
        GError *error = 0; /* initialising ot null avoid trouble... */
        gchar* png_buffer;
        ...
        if(!picture) {
            ...
            gdk_pixbuf_save_to_buffer(picture, &png_buffer, &png_buffer_size, "png", NULL, NULL);

            /* save on disk for test only */
            gdk_pixbuf_save (picture, "picture.png", "png", NULL, NULL); // rm me

            if(ProfileModel::instance().selectedProfile())
                ProfileModel::instance().selectedProfile()->person()->setPhoto(filename);
