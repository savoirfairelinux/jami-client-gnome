...
static void
on_account_creation_failed(RingMainWindow* self)
{
    g_return_if_fail(IS_RING_MAIN_WINDOW(self));
    auto* priv = RING_MAIN_WINDOW_GET_PRIVATE(RING_MAIN_WINDOW(self));
    priv->cpp->isCreatingAccount = false;
...
static void
on_account_creation_started(RingMainWindow* self)
{
    g_return_if_fail(IS_RING_MAIN_WINDOW(self));
    auto* priv = RING_MAIN_WINDOW_GET_PRIVATE(RING_MAIN_WINDOW(self));
...
CppImpl::enterAccountCreationWizard(bool showControls)
{
    if (!widgets->account_creation_wizard) {
        widgets->account_creation_wizard = account_creation_wizard_new(lrc_->getAVModel());
        ...
        g_signal_connect_swapped(widgets->account_creation_wizard, "account-creation-started",
                                 G_CALLBACK(on_account_creation_started), self);
        g_signal_connect_swapped(widgets->account_creation_wizard, "account-creation-failed",
                                 G_CALLBACK(on_account_creation_failed), self);
