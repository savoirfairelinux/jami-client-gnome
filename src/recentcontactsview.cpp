                     ...
                     G_GNUC_UNUSED gpointer data)
{
    ...
    QVariant object = idx.data(static_cast<int>(RecentModel::Role::Object));
                     ...
                     G_GNUC_UNUSED gpointer data)
{
    ...
    if (idx.isValid()) {
        ...
        auto var_lastused = idx.data(static_cast<int>(RecentModel::Role::FormattedLastUsed));
        auto var_status = idx.data(static_cast<int>(RecentModel::Role::Status));
        ...
        if (var_lastused.isValid())
            lastused = var_lastused.value<QString>();
        if (var_status.isValid())
            status = var_status.value<QString>();
        text = g_strdup_printf("%s\n <span fgcolor=\"gray\">time: %s\nstatus: %s</span>",
                                name.toUtf8().constData(),
                                lastused.toUtf8().constData(),
