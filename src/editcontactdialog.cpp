...
static void
save_cb(EditContactDialog *self)
{
...
static void
name_changed(GtkEntry *entry_name, EditContactDialog *self)
{
    g_return_if_fail(IS_EDIT_CONTACT_DIALOG(self));
    EditContactDialogPrivate *priv = EDIT_CONTACT_DIALOG_GET_PRIVATE(self);
...
edit_contact_dialog_init(EditContactDialog *self)
{
     ...
     * the only one */
    GtkTreeIter iter;
    if (gtk_tree_model_get_iter_first(GTK_TREE_MODEL(addressbook_model), &iter)) {
        GValue value = G_VALUE_INIT;
...
edit_contact_dialog_class_init(EditContactDialogClass *klass)
{
    ...
    gtk_widget_class_bind_template_child_private(GTK_WIDGET_CLASS(klass), EditContactDialog, label_uri);
}
