...
static void
create_contact_cb(GtkButton *button, ChooseContactDialog *self)
{
    g_return_if_fail(IS_CHOOSE_CONTACT_DIALOG(self));
    ChooseContactDialogPrivate *priv = CHOOSE_CONTACT_DIALOG_GET_PRIVATE(self);
...
choose_contact_dialog_init(ChooseContactDialog *self)
{
    ...
    g_signal_connect(priv->button_select, "clicked", G_CALLBACK(ok_cb), self);
...
choose_contact_dialog_class_init(ChooseContactDialogClass *klass)
{
    ...
    gtk_widget_class_bind_template_child_private(GTK_WIDGET_CLASS(klass), ChooseContactDialog, button_select);
