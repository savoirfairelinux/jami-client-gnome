...
static void
remove_bootstrap_server(GtkWidget *item, G_GNUC_UNUSED gpointer user_data)
{
    //gpointer data = g_object_get_data(G_OBJECT(item), "bootstrap-server-idx");
    //g_return_if_fail(data);
    g_debug("sWAG SWAG SWAG SWAG");
}
...
static gboolean
bootstrap_servers_popup_menu(G_GNUC_UNUSED GtkWidget *widget, GdkEventButton *event, GtkTreeView *treeview)
{
    /* build popup menu when right clicking on server item */
    ...
    //g_object_set_data(G_OBJECT(remove_server_item), "bootstrap-server-idx", idx);
...
build_tab_view(AccountGeneralTab *view)
{
    ...
    if (priv->account->protocol() != Account::Protocol::RING) {
        ...
        g_signal_connect(GTK_WIDGET(priv->treeview_bootstrap_servers),
                         ...
                         GTK_WIDGET(priv->treeview_bootstrap_servers));
        ...
        gtk_grid_attach(GTK_GRID(priv->grid_parameters), bootstrap_servers_frame, 0, grid_row, 1, 1);
