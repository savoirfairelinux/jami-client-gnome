...
on_resize(GtkWidget *widget, GtkAllocation *allocation, gpointer *self)
{
        ...
        g_debug("got %d, %d", allocation->width, allocation->height);
...
resize_image(SelfieView *self, int height, int width)
{
    ...
    g_debug("new dims %d, %d", (int)((double)gdk_pixbuf_get_width(priv->current_img)*ratio), (int)((double)gdk_pixbuf_get_height(priv->current_img)*ratio));
    ...
    //gtk_container_foreach(GTK_CONTAINER(priv->frame_pic), (GtkCallback) gtk_widget_destroy, NULL);
    priv->current_img_widget = gtk_image_new_from_pixbuf(img);
    gtk_layout_put(GTK_LAYOUT(priv->frame_pic), priv->current_img_widget, 0, 0);
    gtk_widget_set_visible(priv->current_img_widget, true);
}
