...
on_send_button_pressed(SelfieView *self)
{
    ...
    char snapshot_dir[L_tmpnam + 4]; // we will add .jpg at the end
    tmpnam(snapshot_dir);
    strcat(snapshot_dir, ".jpg");

    GError *error = NULL;
    gdk_pixbuf_save (priv->current_img, snapshot_dir, "jpeg", &error, "quality", "100", NULL);

    priv->save_file_name.assign(snapshot_dir);
