...
display_pic(SelfieView *self)
{
    ...
    if (priv->current_img) { // TODO always true at the first time ?
        gtk_container_remove(GTK_CONTAINER(priv->frame_pic), priv->current_img);
    }

    GdkPixbuf *pix = video_widget_get_snapshot(VIDEO_WIDGET(priv->video_widget));
    priv->current_img = gtk_image_new_from_pixbuf(pix);
    gtk_container_add(GTK_CONTAINER(priv->frame_pic), priv->current_img);
    gtk_widget_set_visible(priv->current_img, true);
    // TODO fix size !
}
