...
on_send_button_pressed(SelfieView *self)
{
    ...
    char snapshot_dir[L_tmpnam]; // TODO avoid C array and use C++ strings
    tmpnam(snapshot_dir);

    GError *error = NULL;
    gdk_pixbuf_save (priv->current_img, priv->snapshot_dir, "jpeg", &error, "quality", "100", NULL);

    strcpy(priv->save_file_name, snapshot_dir);
