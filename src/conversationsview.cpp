                     ...
                     gpointer self)
{
    ...
    try
    {
        auto conversation = priv->accountContainer_->accInfo.conversationModel->getConversation(row);
        auto contact = priv->accountContainer_->accInfo.contactModel->getContact(conversation.participants.front());
        std::shared_ptr<GdkPixbuf> image;
        auto var_photo = GlobalInstances::pixmapManipulator().conversationPhoto(
            conversation,
            priv->accountContainer_->accInfo,
            QSize(50, 50),
            contact.isPresent
        );
...
create_and_fill_model(ConversationsView *self)
{
    ...
    // append les rows
    for (auto conversation : priv->accountContainer_->accInfo.conversationModel->getFilteredConversations()) {
        if (conversation.participants.empty()) break; // Should not
        auto contactUri = conversation.participants.front();
        auto contact = priv->accountContainer_->accInfo.contactModel->getContact(contactUri);
...
select_conversation(GtkTreeSelection *selection, ConversationsView *self)
{
    ...
    gtk_tree_model_get(model, &iter,
                       ...
                       -1);
    auto priv = CONVERSATIONS_VIEW_GET_PRIVATE(self);
    priv->accountContainer_->accInfo.conversationModel->selectConversation(std::string(conversationUid));
}
...
build_conversations_view(ConversationsView *self)
{
    ...
    &*priv->accountContainer_->accInfo.conversationModel,
