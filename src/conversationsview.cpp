                     ...
                     gpointer self)
{
    ...
    auto isBanned = false;
    if (not convOpt->participants.empty()) {
                                 ...
                                 GtkTreeView *treeview)
{
    ...
    auto isBanned = false;
    if (not convOpt->participants.empty()) {
        auto contactUri = convOpt->participants.front();
            ...
            G_GNUC_UNUSED GtkTreeView *treeview)
{
    ...
    try
    {
        ...
        auto& interactions = convOpt->interactions;
        auto lastUid = convOpt->lastMessageUid;
...
update_conversation(ConversationsView *self, const std::string& uid) {
    ...
    while(iterIsCorrect) {
        ...
        if(std::string(uid) == uidModel) {
            ...
            if (convOpt->participants.empty()) {
                g_free(uidModel);
                return;
            }
            auto contactUri = convOpt->participants.front();
            auto contactInfo = (*priv->accountInfo_)->contactModel->getContact(contactUri);
            auto lastMessage = convOpt->interactions.empty() ? "" :
                convOpt->interactions.at(convOpt->lastMessageUid).body;
            ...
            gtk_list_store_set (GTK_LIST_STORE(model), &iter,
                                0 /* col # */ , qUtf8Printable(convOpt->uid) /* celldata */,
...
create_and_fill_model(ConversationsView *self)
{
    ...
    fillModelLambda((*priv->accountInfo_)->conversationModel->getAllSearchResults().get());
