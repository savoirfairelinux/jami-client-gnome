...
static void
on_account_creation_completed(AccountView *view)
{
    g_return_if_fail(IS_ACCOUNT_VIEW(view));
    AccountViewPrivate *priv = ACCOUNT_VIEW_GET_PRIVATE(view);
...
show_account_creation_wizard(AccountView *view)
{
    ...
    if (!account_creation_wizard)
    {
        account_creation_wizard = account_creation_wizard_new();
        g_signal_connect_swapped(account_creation_wizard, "account-creation-completed", G_CALLBACK(on_account_creation_completed), view);
    ...
    gtk_widget_show_all(account_creation_wizard);
