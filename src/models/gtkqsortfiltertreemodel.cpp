        ...
        [=](const QModelIndex & sourceParent, int sourceStart, int sourceEnd,
            ...
            if (sourceParent == destinationParent) {
                ...
                if (sourceParent.isValid()) {
                    auto idx_original = retval->priv->given_model->mapToSource(sourceParent);
                    auto idx_access = retval->priv->access_model->mapFromSource(idx_original);
                    iter_parent = g_new0(GtkTreeIter, 1);
                    iter_parent->stamp = stamp;
                    qmodelindex_to_iter(idx_access, iter_parent);
                    path_parent = gtk_q_sort_filter_tree_model_get_path(GTK_TREE_MODEL(retval), iter_parent);
                } else {
