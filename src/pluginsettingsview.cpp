...
static GtkWidget*
get_revoke_button_from_row(GtkWidget* row)
{
...
static void
revoke_plugin(GtkButton* button, PluginSettingsView *view)
{
    ...
    while (GtkWidget* children = GTK_WIDGET(gtk_list_box_get_row_at_index(GTK_LIST_BOX(priv->list_installed_plugins), row))) {
        if (GTK_BUTTON(get_revoke_button_from_row(children)) == button) {
            ...
            auto* password_dialog = gtk_message_dialog_new(top_window,
                ...
                _("Warning! This action will revoke the plugin!\nNote: this action cannot be undone."));
            gtk_window_set_title(GTK_WINDOW(password_dialog), _("Revoke Plugin"));
...
add_plugin(PluginSettingsView *view, const QString plugin)
{
    ...
    GdkPixbuf* pluginIcon = gdk_pixbuf_new_from_file_at_size((details.iconPath).toStdString().c_str(), 30, 30, NULL);
    ...
    gtk_box_pack_start(GTK_BOX(plugin_info_box), label_name, false, false, 0);
    gtk_widget_set_halign(label_name, GtkAlign::GTK_ALIGN_START);
    auto* label_id = gtk_label_new("");
    gtk_box_pack_start(GTK_BOX(plugin_info_box), label_id, false, false, 0);
    gtk_box_pack_start(GTK_BOX(plugin_box), plugin_info_box, false, false, 0);
    
    // Add uninstall button
    auto image = gtk_image_new_from_icon_name("dialog-error-symbolic", GTK_ICON_SIZE_LARGE_TOOLBAR);
    auto* uninstall_plugin_button = gtk_button_new();
    gtk_button_set_relief(GTK_BUTTON(uninstall_plugin_button), GTK_RELIEF_NONE);
    gtk_widget_set_tooltip_text(uninstall_plugin_button, _("Revoke Plugin"));
    ...
    g_signal_connect(uninstall_plugin_button, "clicked", G_CALLBACK(revoke_plugin), view);
