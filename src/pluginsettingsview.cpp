...
add_plugin(PluginSettingsView *view, const QString plugin)
{
    ...
    gtk_box_pack_start(GTK_BOX(device_box), GTK_WIDGET(aux), false, false, 0);
    ...
    // std::string markup = "<span font_desc=\"7.0\">" + (details.path).toStdString() + "</span>";
    ...
    gtk_widget_set_tooltip_text(action_device_button, _("Revoke Plugin"));
    ...
    g_signal_connect(action_device_button, "clicked", G_CALLBACK(revoke_plugin), view);
    gtk_box_pack_end(GTK_BOX(device_box), GTK_WIDGET(action_device_button), false, false, 0);
    // Insert at the end of the list
    gtk_list_box_insert(GTK_LIST_BOX(priv->list_installed_plugins), device_box, -1);
