...
ring_welcome_view_init(RingWelcomeView *self)
{
    ...
    /* QR code */
    priv->rcode = QRcode_encodeString(gtk_label_get_text((GtkLabel*)label_ringid),
    ...
    auto signal_id = g_signal_connect(qr_ringid, "draw", G_CALLBACK(draw_qrcode), priv);
    ...
    priv-> ringaccount_updated = QObject::connect(
        ...
        [label_explanation, label_ringid, priv] () {
...
draw_qrcode(GtkWidget* diese,
            ...
            gpointer   data)
{
    auto priv = (RingWelcomeViewPrivate*) data;

    if (!priv->rcode) // no rcode, no draw
        return TRUE;

    auto margin = 5;
    auto qrsize = 200;
    int qrwidth = priv->rcode->width + margin * 2;
    ...
    for(int y = 0; y < priv->rcode->width; y++) {
        row = (p + (y * priv->rcode->width));
        for(int x = 0; x < priv->rcode->width; x++) {
            if(*(row + x) & 0x1) {
