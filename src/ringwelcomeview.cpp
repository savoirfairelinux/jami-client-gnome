...
ring_welcome_update_view(RingWelcomeView* self) {
    ...
    if (!*priv->accountInfo_) {
        g_debug("Called welcome_update_view but NULL Info pointer");
        return;
    }

    // Only draw a basic view for SIP accounts
    if ((*priv->accountInfo_)->profileInfo.type == lrc::api::profile::Type::SIP) {
...
GtkWidget *
ring_welcome_view_new(const lrc::api::account::Info*& accountInfo)
{
